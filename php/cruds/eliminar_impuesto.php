...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_GET['id'])) {
  ...
  try {
    ...
    $tabla_catalogo = "impuestos";
    $columna_id = "idimpuesto";

    // Configura dónde vas a buscar si este registro está en uso
    $tabla_relacionada = "productos";
    $columna_foranea = "id_impuesto";  // La columna en la tabla relacionada que guarda este ID
    ...
    if ($usoData['total'] > 0) {
      $response["message"] = "No se puede eliminar. Este registro está siendo utilizado por " . $usoData['total'] . " elemento(s) en el sistema (ej. Productos o ordenes de servicio). Cámbielo a estado Inactivo.";
      echo json_encode($response);
      exit;
    }
