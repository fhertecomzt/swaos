...
try {
  ...
  if (!empty($data['tipoequipo'])) {
    $ts = trim($data['tipoequipo']); // Limpiar el nombre
    $id = isset($data['id']) ? intval($data['id']) : 0; // ID (opcional)

    // Si se proporciona un ID (edición), excluir ese registro de la validación
    if ($id > 0) {
      $query = "SELECT COUNT(*) FROM equipos WHERE nombre_equipo = ? AND id_equipo != ?";
      $stmt = $dbh->prepare($query);
      $stmt->execute([$ts, $id]);
    } else {
      // Si no hay ID, es un registro nuevo
      $query = "SELECT COUNT(*) FROM equipos WHERE nombre_equipo = ?";
      $stmt = $dbh->prepare($query);
      $stmt->execute([$ts]);
    }
