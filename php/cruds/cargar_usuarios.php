...
// Filtro por estatus
if ($estatus === 'activo') {
  $sql .= " WHERE u.estatus = 0 AND r.nomrol != 'SISTEMAS' ORDER BY u.idusuario ASC";
} elseif ($estatus === 'inactivo') {
  $sql .= " WHERE u.estatus = 1 AND r.nomrol != 'SISTEMAS' ORDER BY u.idusuario ASC";
} else {
  // Si es "todos", también omitir el rol "SISTEMAS"
  $sql .= " WHERE r.nomrol != 'SISTEMAS' ORDER BY u.idusuario ASC" ;
}
