...
try {
  ...
  if (!empty($data['estadoservicio'])) {
    $estadoservicio = trim($data['estadoservicio']); // Limpiar el nombre
    $id = isset($data['id']) ? intval($data['id']) : 0; // ID (opcional)

    // Si se proporciona un ID (edición), excluir ese registro de la validación
    if ($id > 0) {
      $query = "SELECT COUNT(*) FROM estadosservicios WHERE estado_servicio = ? AND id_estado_servicio != ?";
      $stmt = $dbh->prepare($query);
      $stmt->execute([$estadoservicio, $id]);
    } else {
      // Si no hay ID, es un registro nuevo
      $query = "SELECT COUNT(*) FROM estadosservicios WHERE estado_servicio = ?";
      $stmt = $dbh->prepare($query);
      $stmt->execute([$estadoservicio]);
    }

    // Actualizar la respuesta en función del resultado
    $response["existe"] = $stmt->fetchColumn() > 0;
  } else {
    $response["error"] = "Falta el parámetro 'estadoservicio'.";
  }
} catch (Exception $e) {
