...
try {
  ...
  if (!empty($data['impuesto'])) {
    $impuesto = trim($data['impuesto']); // Limpiar el nombre
    $id = isset($data['id']) ? intval($data['id']) : 0; // ID (opcional)

    // Si se proporciona un ID (edición), excluir ese registro de la validación
    if ($id > 0) {
      $query = "SELECT COUNT(*) FROM impuestos WHERE nomimpuesto = ? AND idimpuesto != ?";
      $stmt = $dbh->prepare($query);
      $stmt->execute([$impuesto, $id]);
    } else {
      // Si no hay ID, es un registro nuevo
      $query = "SELECT COUNT(*) FROM impuestos WHERE nomimpuesto = ?";
      $stmt = $dbh->prepare($query);
      $stmt->execute([$impuesto]);
    }

    // Actualizar la respuesta en función del resultado
    $response["existe"] = $stmt->fetchColumn() > 0;
  } else {
    $response["error"] = "Falta el parámetro 'impuesto'.";
  }
} catch (Exception $e) {
